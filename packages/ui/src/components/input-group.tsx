...
const inputGroupAddonVariants = cva(
  "[&_svg]:-mx-0.5 flex h-auto cursor-text select-none items-center justify-center gap-2 [&>kbd]:rounded-[calc(var(--radius)-5px)] in-[[data-slot=input-group]:has([data-slot=input-control],[data-slot=textarea-control])]:[&_svg:not([class*='size-'])]:size-4.5 sm:in-[[data-slot=input-group]:has([data-slot=input-control],[data-slot=textarea-control])]:[&_svg:not([class*='size-'])]:size-4 not-has-[button]:**:[svg:not([class*='opacity-'])]:opacity-80",
  {
    ...
    variants: {
      align: {
        "block-end":
          "order-last w-full justify-start px-[calc(var(--spacing)*3-1px)] pb-[calc(var(--spacing)*3-1px)] [.border-t]:pt-[calc(var(--spacing)*3-1px)] [[data-size=sm]+&]:px-[calc(var(--spacing)*2.5-1px)]",
        "block-start":
          "order-first w-full justify-start px-[calc(var(--spacing)*3-1px)] pt-[calc(var(--spacing)*3-1px)] [.border-b]:pb-[calc(var(--spacing)*3-1px)] [[data-size=sm]+&]:px-[calc(var(--spacing)*2.5-1px)]",
        "inline-end":
          "has-[>:last-child[data-slot=badge]]:-me-1.5 has-[>button]:-me-2 order-last pe-[calc(var(--spacing)*3-1px)] has-[>kbd:last-child]:me-[-0.35rem] [[data-size=sm]+&]:pe-[calc(var(--spacing)*2.5-1px)]",
        "inline-start":
          "has-[>:last-child[data-slot=badge]]:-ms-1.5 has-[>button]:-ms-2 order-first ps-[calc(var(--spacing)*3-1px)] has-[>kbd:last-child]:ms-[-0.35rem] [[data-size=sm]+&]:ps-[calc(var(--spacing)*2.5-1px)]",
      },
    },
  }
);
