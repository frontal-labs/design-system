...
const cloneAI = async (name: string, _packageManager: string) => {
...
const setupEnvironmentVariables = async () => {
	...
	// Check for .env.example files in common locations
	const commonPaths = [
		{ source: ".", target: ".env.local" },
		{ source: "packages", target: ".env.local" },
	];

	for (const { source, target } of commonPaths) {
		const examplePath = join(source, ".env.example");
		try {
...
const setupBuild = async (packageManager: string) => {
	const _filterCommand = packageManager === "npm" ? "--workspace" : "--filter";

	const command = [packageManager, "run", "build"].join(" ");
...
export const initialize = async (options: {
	...
	try {
		...
		await setupBuild(packageManager);
		...
		outro(
			"Please review the documentation and configure your environment variables before starting the project.",
		);
	} catch (error) {
		const message =
